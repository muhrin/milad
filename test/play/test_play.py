...
def test_env_simple_no_cutoff_fn(geometric_invariants, request, save_figures):
    ...
    for _ in range(num_rotations):
        ...
        series.append(
            rotated_env.calc_moment_invariants(
                geometric_invariants, normalise=normalise
            )
        )
...
def test_env_simple_cos_cutoff_fn(geometric_invariants, request, save_figures):
    ...
    for _ in range(num_rotations):
        ...
        _results = rotated_env.add_gaussians(rotated, sigma)  # noqa: F841

        # Calculate the invariants
        series.append(
            rotated_env.calc_moment_invariants(geometric_invariants, normalise=True)
        )
...
def test_atom_entering(geometric_invariants, request, save_figures):
    ...
    target = num_atoms - 1
    positions[target] = (start_x, 0.0, 0.0)
    ...
    for i in range(num_steps):
        ...
        positions[target] = np.array((x_pos, 0.0, 0.0))
        ...
        # Calculate the invariants
        series.append(
            env.calc_moment_invariants(geometric_invariants, normalise=normalise)
        )
...
def test_fingerprint(geometric_invariants, request, save_figures):
    ...
    calculator = milad.play.FingerprintCalculator(
        invariants=geometric_invariants,
        sigma=1.0,
        cutoff=6.0,
        cutoff_function="cos",
        normalise=True,
    ...
    milad.plot.plot_multiple_invariants(
        series, axes, labels=["$R_{{{}}}$".format(idx) for idx in range(len(series))]
    )
    axes.set_xlabel(
        "Total fingerprint ({} atoms x {} invariants)".format(num_atoms, num_invariants)
    )
    fig.legend()
    if save_figures:
