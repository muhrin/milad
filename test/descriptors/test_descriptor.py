...
def test_descriptor_derivatives():
    ...
    descriptor = milad.descriptor(
        features=dict(type=functions.WeightedDelta, map_species_to=None),
        cutoff=5.0,
        apply_cutoff=False,
        invs=invs,
    )

    structure = atomic.random_atom_collection_in_sphere(1, 5.0)
...
def test_descriptor_smoothness(descriptor):
    ...
    for x in np.linspace(1.01, 0.9, 12):
        atoms = milad.AtomsCollection(1, positions=[x, 0.0, 0.0], numbers=1)
        fingerprint = descriptor(atoms)

        fingerprint_max = np.abs(fingerprint).max()

        if x >= 1.0:
            assert fingerprint_max == 0.0
