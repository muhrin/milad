...
def lj_training_data():
    training_size = 50
    r = np.linspace(0.3, 1.0, num=training_size)
    ...
    training_forces = np.array(
        [force * dr for force, dr in zip(map(lennard_jones_force, r), drs)]
    )

    training_data = []
    for dr, energy, force in zip(drs, training_energies, training_forces):
        atoms = ase.Atoms(positions=[[0.0, 0.0, 0.0], dr], numbers=[6, 6])
        calc = singlepoint.SinglePointCalculator(
            atoms, energy=energy, forces=[-force, force]
        )
        atoms.calc = calc
...
def test_fitting_data(descriptor, training_data):
    fingerprint_set = dat.create_fingerprint_set(
        descriptor, training_data, get_derivatives=True
    )
    fitting_data = neuralnetwork.FittingData.from_fingerprint_set(
        fingerprint_set, requires_grad=True
    )
    ...
    for system_idx, system in enumerate(fitting_data.iter_systemwise()):
        ...
        assert torch.all(
            fitting_data.fingerprints[idx : idx + natoms] == system.fingerprints
        )
    ...
    for idx, atoms in enumerate(training_data):
        ...
        assert np.isclose(
            fitting_data.total_energies[idx].item(), atoms.get_potential_energy()
        )
        assert np.isclose(
            fitting_data.get_normalised_energies()[idx].item(),
            atoms.get_potential_energy() / natoms,
        )
        assert np.all(
            fitting_data.index[total_atoms : total_atoms + natoms].detach().numpy()
            == idx
        )
...
def test_neural_network_basics(descriptor, training_data):
    ...
    fitting_data = neuralnetwork.FittingData.from_fingerprint_set(
        fingerprint_set, requires_grad=True, device=nn.device
    )
    ...
    assert len(predictions.forces) == len(
        fitting_data.fingerprints
    ), "Expect one force per environment"
...
def test_predictions(descriptor, training_data):
    ...
    env_energies = torch.zeros(
        (fitting_data.index.shape[0], 1), dtype=fitting_data.total_energies.dtype
    )
    total_atoms = 0
    ...
    pred = neuralnetwork.Predictions(
        fitting_data.num_atoms, fitting_data.index, env_energies
    )
    ...
    assert torch.isclose(loss.energy, torch.tensor(0.0))
    assert torch.isclose(loss.total, torch.tensor(0.0))
...
def test_nn_lennard_jones(descriptor, lj_training_data):
    ...
    fingerprint_set = dat.create_fingerprint_set(
        descriptor, lj_training_data, get_derivatives=True
    )
    _training, _loss_fn = nn.fit(
        fingerprint_set, max_epochs=50, batchsize=1000, learning_rate=1e-3
    )
