...
def geometric_invariants():
    ...
    invs = milad.invariants.read(
        filename=invariants.GEOMETRIC_INVARIANTS, read_max=READ_MAX
    )
    return invs
...
def create_fake_training_data(num_systems=10, max_atoms=18, species=("Si",)):
    ...
    for _ in range(num_systems):
        ...
        atoms = ase.Atoms(
            positions=generate.random_points_in_sphere(natoms),
            symbols=random.choices(species, k=natoms),
        )
        calc = SinglePointCalculator(
            atoms, energy=-100 * random.random(), forces=np.random.rand(natoms, 3)
        )
        atoms.calc = calc
