...
def test_map_numbers():
    ...
    system = atomic.AtomsCollection(
        ...
        numbers=np.array([1, 2, 5, 2, 3, 8]),
    )
    numbers = set(system.numbers)

    # Map the five species into the range 2 -> 3
    mapper = atomic.MapNumbers(numbers, map_to=(2.0, 3.0))
...
def test_separation_force():
    ...
    separation = 0.5
    atoms = atomic.AtomsCollection(
        2, positions=[[-0.5 * separation, 0, 0], [0.5 * separation, 0, 0]], numbers=1.0
    )
    ...
    assert (
        energy > 0.0
    ), "The energy should always be positive if the separation less than the cutoff"

    force = separation_force.force(separation)
    assert force > 0.0, "The force should act to separate the two atoms"

    assert np.all(jacobian[0, atoms.linear_pos_idx(0)] == np.array([force, 0.0, 0.0]))
    assert np.all(jacobian[0, atoms.linear_pos_idx(1)] == np.array([-force, 0.0, 0.0]))
...
def test_separation_force_symbolic():
    ...
    assert energy_at_rcut == 0.0

    force_at_rcut = force.force(rcut)
    assert force_at_rcut == 0.0
...
def test_separation_force_optimiser():
    ...
    res = optimisers.LeastSquaresOptimiser().optimise(
        force, initial=atoms, verbose=True
    )
...
def test_scale_positions():
    atoms = atomic.random_atom_collection_in_sphere(10, radius=5.0, centre=True)
    testing.test_function(atomic.ScalePositions(0.5), atoms, check_jacobian=True)


def test_feature_mapper():
    atoms = atomic.random_atom_collection_in_sphere(
        10, numbers=tuple(random.randint(0, 100) for _ in range(10))
    )
    testing.test_function(
        atomic.FeatureMapper(map_species_to="WEIGHT"), atoms, check_jacobian=True
    )
