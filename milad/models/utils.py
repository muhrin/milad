...
class TrainingMonitor:
    ...
    class PlottingDataset:
        ...
        def replot(self):
            ...
            self._last_plt = self._axes.scatter(
                self._steps, self._values, **self._scatter_kwargs
            )

    def __init__(
        self,
        validation_data: neuralnetwork.FittingData = None,
        show_validation_every=10,
    ):
        ...
        self._energy_validation = TrainingMonitor.PlottingDataset(
            self.energy_axis,
            scatter_kwargs=dict(c="tab:orange", alpha=0.6, label="Energy (validation)"),
        )
        ...
        self._force_training = TrainingMonitor.PlottingDataset(
            ...
            scatter_kwargs=dict(
                ...
                alpha=0.6,
                facecolors="none",
                label="Force (training)",
            ),
        )
        self._force_validation = TrainingMonitor.PlottingDataset(
            forces_axis,
            scatter_kwargs=dict(c="tab:blue", alpha=0.6, label="Force (validation)"),
        )
    ...
    def plot_energy_comparison(
        ...
        *fitting_data: neuralnetwork.FittingData
        ...
        if training_data:
            ...
            known_energies = (
                training_data.get_normalised_energies().cpu().detach().numpy()
            )
            axis.scatter(
                known_energies,
                predictions.get_normalised_energies().cpu().detach().numpy(),
                c="r",
                alpha=0.3,
            )
        ...
        if self._validation_data:
            ...
            known_energies = (
                self._validation_data.get_normalised_energies().cpu().detach().numpy()
            )
            axis.scatter(
                known_energies,
                predictions.get_normalised_energies().cpu().detach().numpy(),
    ...
    def plot_energy_deviation_histogram(
        self, network, *fitting_data: neuralnetwork.FittingData, bins=100
    ):
        ...
        for data_set in all_datasets:
            ...
            predicted_energies = (
                network.make_prediction(data_set)
                .get_normalised_energies()
                .cpu()
                .detach()
                .numpy()
            )
