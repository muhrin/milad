...
__all__ = (
    "ZernikeMoments",
    "ZernikeMomentsCalculator",
    "ZernikeMomentCalculator",
    "from_deltas",
    "from_gaussians",
)

NORMALISTAION = 3.0 / (4.0 * np.pi)
            ...
            for l in inclusive(n):
                ...
                for m in inclusive(-l, l):
                    vals = (
                        norm
                        * zernike_poly(
                            n, l, m, spherical, condon_shortley=False
                        ).conjugate()
                    )
                    zernike_moms.array[n, l, m] = np.sum(weights * vals)
    else:
        zernike_moms.array.fill(0.0)
...
def from_geometric_moments(
    n_max: int,
    geom_moments: Union[geometric.GeometricMoments, np.array],
    l_max: int = None,
) -> "ZernikeMoments":
...
class ZernikeReconstructionQuery(base_moments.ReconstructionQuery):
    ...
    def moments_in_domain(self) -> np.ndarray:
        if self._moments_in_domain is None:
            ...
            self._moments_in_domain = self._moments[:, :, :, self.valid_idxs][
                :, :, :, :, 0
            ]
...
class ZernikeMoments(base_moments.Moments):
    ...
    def from_vector(cls, n_max: int, vec: np.array, redundant=True) -> "ZernikeMoments":
        ...
        for (n, l, m), value in zip(iter_indices(redundant=redundant), vec):
            if n > n_max:
                break
            moms._set_moment(
                n, l, m, value, set_minus_m=not redundant
            )  # pylint: disable=protected-access

        return moms

    @classmethod
    def rand(cls, n_max: int, num_range=(-5.0, 5.0)):
    ...
    def __init__(self, n_max: int, l_max=None, dtype=complex):
        ...
        idx_array = ma.masked_array(
            np.empty(moments.shape, dtype=int), mask=moments.mask
        )
                ...
                try:
                    if value.imag > 1e-9:
                        logging.warning(
                            ...
                            "however these moment should always be real so discarding imaginary",
                            n,
                            l,
                            m,
                            value,
    ...
    def value_at(self, x: np.array, order: int = None) -> float:
        ...
        return self.reconstruct(
            self.create_reconstruction_query(x, order), order, zero_outside_domain=False
        )

    def reconstruct(
        self, query: ZernikeReconstructionQuery, order=None, zero_outside_domain=True
    ...
    def visualise(
        ...
        if viewer.startswith("plotly_widget"):
            ...
            default_isomin = (
                grid_values.max() - grid_values.min()
            ) * 0.6 + grid_values.min()
            ...
            def update(
                opacity=default_opacity, isomin=default_isomin, isomax=default_isomax
            ):
    ...
    def create_reconstruction_query(
        cls, points: np.ndarray, max_order: int
    ) -> ZernikeReconstructionQuery:
        ...
        """
        return ZernikeReconstructionQuery(
            max_order, points, cls._get_geometric_moments(points, max_order)
        )
...
class ZernikeMomentsBuilder(functions.Function):
    ...
    def evaluate(
        self, state: np.ndarray, *, get_jacobian=False
        ...
        jac = (
            np.zeros((len(moms), len(indices_list)), dtype=complex)
            if get_jacobian
            else None
        )
        ...
        for idx, (zernike_idx, num_type) in enumerate(indices_list):
            ...
            if get_jacobian:
                ...
                if zernike_idx.m > 0:
                    ...
                    # pylint: disable=unsupported-assignment-operation
                    jac[lin_idx, idx] += (-1) ** m * (1 if num_type == 1.0 else -1j)
    ...
    class Inverse(functions.Function):
        ...
        def evaluate(  # pylint: disable=unused-argument
            self, state: ZernikeMoments, *, get_jacobian=False
        ) -> np.ndarray:
            moms = []
            for indices, num_type in self._forward.iter_indices():
                mom = state[indices]
                moms.append(mom.real if num_type == 1.0 else mom.imag)
...
class ZernikeMomentsCalculator(base_moments.MomentsCalculator):
    ...
    _change_of_basis_polys = (
        dict()
    )  # Change of basis polynomials (geometric -> spherical harmonic)

    @classmethod
    def change_of_basis(
        cls, n: int, l: int, m: int
    ) -> polynomials.HomogenousPolynomial:
        try:
    ...
    def evaluate(
    ...
    ) -> Union[ZernikeMoments, Tuple[ZernikeMoments, np.ndarray]]:
        if self._use_direct:
            if not isinstance(state, functions.Features) or not all(
                isinstance(feature, functions.WeightedDelta) for feature in state
            ):
                raise ValueError(
                    "Can only use direct method on a set of features that are delta functions"
                )
        ...
        moments = ZernikeMoments(
            self._n_max,
            self._l_max,
            dtype=object if geom_moments.dtype == object else complex,
        )
...
def get_jacobian_wrt_geom_moments(max_order: int, redundant=True):
    ...
    class DerivativeTracker:
        ...
        def __getitem__(self, item: base_moments.Index):
            ...
            try:
                vector[geometric.linear_index(self._max_order, item)] = 1.0
            except IndexError:
                raise IndexError(
    ...
    num_moments = ZernikeMoments.num_moments(
        max_order, redundant=redundant
...
def sum_chi_nlm(
    n: int, l: int, m: int, geom_moments: base_moments.MomentsProto
...
def sum2(
    n: int, l: int, m: int, nu: int, geom_moments: base_moments.MomentsProto
) -> Union[complex, np.array]:
    total = 0.0
    for alpha in inclusive(nu):
        total += binomial_coeff(nu, alpha) * sum3(
            n=n, l=l, m=m, nu=nu, alpha=alpha, geom_moments=geom_moments
        )
    return total


def sum3(
    n: int, l: int, m: int, nu: int, alpha: int, geom_moments: base_moments.MomentsProto
) -> Union[complex, np.array]:
    total = 0.0
    for beta in inclusive(nu - alpha):
        total += binomial_coeff(nu - alpha, beta) * sum4(
            n=n, l=l, m=m, nu=nu, alpha=alpha, beta=beta, geom_moments=geom_moments
        )
    return total
...
def sum4(
    ...
    alpha: int,
    beta: int,
    geom_moments: base_moments.MomentsProto,
) -> Union[complex, np.array]:
    total = 0.0
...
def sum5(
    ...
    total = 0.0
    for mu in inclusive(int(math.floor((l - m) / 2.0))):
        total += (
            (-1) ** mu
            * 2 ** (-2 * mu)
            * binomial_coeff(l, mu)
...
def omega_nl_m(
    ...
    return (
        NORMALISTAION
        * ZernikeMomentsCalculator.change_of_basis(n, l, m)(geom_moments).conjugate()
    )


def iter_indices(
    n_max: int = None, l_max: int = None, *, redundant=False
) -> Iterator[ZernikeIndex]:
...
def iter_indices_extended(
    key: Tuple[int, Tuple], max_order: int, redundant=False
) -> Iterator[ZernikeIndex]:
