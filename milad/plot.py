...
def plot_invariants(
    values, axes: matplotlib.pyplot.Axes = None, label=None, scatter=True
):
    if axes is None:
    ...
    if scatter:
        axes.scatter(numpy.array(range(len(values))), values, s=5.0)

    return axes


def plot_multiple_invariants(
    series, axes: matplotlib.pyplot.Axes = None, labels="auto"
):
    if axes is None:
        _fig, axes = matplotlib.pyplot.subplots()
